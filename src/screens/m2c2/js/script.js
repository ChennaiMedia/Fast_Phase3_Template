...
$(document).ready(function(){
	...
	$('.btn-text').on('click',function(){
		...
		if(targetId==0)
		{
			...
			$('.contentStep').text('Update the Cash Flow Statement to show that the first incoming cash is the 14000 invested by the shareholders. Type 14000 into the box under ‘Cash Flow’. The Cash balance will update automatically. Then select Submit. This will check that you have recorded the transaction correctly. ​');
	...
	$('.submit').on('click',function(){
		...
		if(JSON.stringify(getAnswet)==JSON.stringify(correctAnswer)) {
			...
			setTimeout(function(){$('.feedback-holder').show();$('.pop_up').show();},200);
			complete_page();
			enableNextBtn();
